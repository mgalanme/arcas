...
# ARCAS - 01_ingestion_daily v4
# Cambios v4:
#   - Nivel 1: prompt con calidad probatoria, velocidad procesal, credibilidad de fuente
#   - Neo4j Aura: extraccion de entidades y relaciones
...
def groq_invoke(prompt, max_tokens=500, temperature=0.1):
    for attempt in range(3):
        try:
            ...
            log.warning(f"Groq attempt {attempt+1}: {e}"); time.sleep(5)
    return ""
...
HEADERS_HTTP = {
    "User-Agent":"Mozilla/5.0 (compatible; ARCAS-Research/4.0)",
...
def fetch_boe(pub_date):
    ...
    for item in root.findall(".//item"):
        ...
        records.append({
            ...
            "content_url":url_h or url_p,"content_snippet":f"{dept}: {title}" if dept else title,
...
print(f"New: {len(new_records)} / Total: {len(all_records)}")
...
KW_CB=["archivo","archiva","sobresee","prescripcion","dilaciones","sin pruebas",
       ...
       "nulidad","cautelar","acusacion particular"]
KW_CP=["psoe","pp","vox","podemos","partido socialista","partido popular",
       "ciudadanos","sumar","junts","pnv","afiliado","militante"]
...
def score(title, source_type, is_fc):
    ...
    def jbias():
        ...
        return 0.0
    return {
        "cat_a":min(kw(KW_A)+(0.15 if source_type=="gazette" else 0),0.9),
        "cat_b":kw(KW_B),"cat_c":jbias(),
        "cat_d":max(kw(KW_D),0.45 if is_fc else 0),
        "cat_e":kw(KW_E),"cat_f":kw(KW_F),
    }

def top_cat(scores): return max(scores, key=scores.get).replace("cat_","").upper()

candidates = [r for r in (new_records or []) if r.get("title")]
...
Analiza con MAXIMO RIGOR y desde perspectiva POLITICAMENTE NEUTRAL.
...
Responde en espanol con esta estructura exacta:

**PATRON DETECTADO:**
[Describe el patron. Si no hay evidencia real, indicalo explicitamente.]

**CALIDAD DE LAS PRUEBAS:**
Tipo: [Documentos oficiales / Testimonios / Declaraciones / Recortes de prensa / Suposiciones / Sin pruebas]
Nivel probatorio: [Alto-pruebas materiales | Medio-testimonios o indicios | Bajo-acusaciones sin respaldo | Nulo-sin evidencia]
[Explica en que te basas concretamente.]

**VELOCIDAD PROCESAL:**
[Si hay caso judicial: es llamativamente rapido o lento vs casos similares? Se menciona alguna comparativa temporal? Si no aplica, escribe No procede.]

**SESGO DE LA FUENTE:**
[Esta fuente tiene historial de bulos? La noticia aparece solo en medios de un espectro o esta contrastada por fuentes diversas? Se explicito.]

**ACTORES IMPLICADOS:**
[Lista: nombre - rol - partido/organismo si se menciona]

**NIVEL DE PREOCUPACION:** [Alto / Medio / Bajo / Sin evidencia]

**ACCION CIUDADANA RECOMENDADA:**
[Que documentos pedir, que registros consultar para verificar de forma independiente.]"""

ENTITY_PROMPT = """Extrae entidades del siguiente texto en JSON valido.
SOLO JSON, sin explicaciones. Formato: {{"personas":[],"organismos":[],"casos":[],"partidos":[]}}
...
for record, scores in above[:limit]:
    ...
    analysis = groq_invoke(DEEP_PROMPT.format(
        ...
        snippet=snippet[:800] if snippet else "(sin contenido disponible)",
    ), max_tokens=600)
    if not analysis: analysis = f"[Sin analisis] Categoria {cat}: {title_es[:100]}"

    # Ajustar confianza segun calidad probatoria y credibilidad de fuente
    conf = base_conf + 0.10
    al = analysis.lower()
    if "nivel probatorio: nulo" in al or "sin evidencia" in al: conf -= 0.20
    elif "nivel probatorio: bajo" in al: conf -= 0.10
    elif "nivel probatorio: alto" in al: conf += 0.10
...
if NEO4J_URI and entities_to_save:
    try:
        ...
        def upsert_entity(tx, e):
            tx.run("""
                MERGE (n:Entity {entity_id: $eid})
                ON CREATE SET n.name=$name, n.type=$etype,
                              n.first_seen=$fs, n.mention_count=1
                ON MATCH SET  n.last_seen=$ls,
                              n.mention_count=n.mention_count+1
            """, eid=e["entity_id"],name=e["entity_name"],
                etype=e["entity_type"],fs=e["first_seen"],ls=e["last_seen"])

        def link_entities(tx, e, source, alert_id):
            tx.run("""
            ...
            """, src=source, aid=alert_id, eid=e["entity_id"])

        with driver.session() as sess:
            for e in entities_to_save:
                sess.execute_write(upsert_entity, e)
                sess.execute_write(link_entities, e, e["sources"], e["alert_ids"])
        driver.close()
        print(f"Neo4j: upserted {len(entities_to_save)} entities")
...
print(f"\n=== ARCAS Daily Run v4 ===")
print(f"Articulos almacenados:  {ta}")
